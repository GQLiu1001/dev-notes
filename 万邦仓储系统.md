...
// 登出函数也很简单，不需要手动设置 tokenexport const logout = () => {  
...
export const fetchSalesTrend = () => {  
    ...
    const startMonthStr = startMonth.toISOString().slice(0, 7); // 格式化为 "YYYY-MM"    const endMonthStr = endMonth.toISOString().slice(0, 7);  
...
// 定义 UpdateUserRequest 类型，包含 oldPasswordinterface UpdateUserRequest extends Omit<User, 'id' | 'role_id' | 'description'> {  
    oldPassword?: string;  
}  
