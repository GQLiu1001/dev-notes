# 万邦仓储系统
### 1.3 旧管理端增加模块
- [ ] 司机审核模块，显示司机姓名手机号，有审核通过按钮和删除按钮  
- [ ] 订单派送/订单派送信息模块 在订单后有按钮显示派送，点击后输入重量与地址与配送费（自动导入客户姓名手机号）发出订单（Rabbitmq Redisson）共四个状态：派单 -》待接单 -》配送中 -》配送完成 如果订单被取消，管理端提示派送订单已被取消并恢复状态到派单。 ### 1.3 旧管理端增加模块
- [ ] 司机审核模块，显示司机姓名手机号，有审核通过按钮和删除按钮
- [ ] 订单派送/订单派送信息模块 在订单后有按钮显示派送，点击后输入重量与地址与配送费（自动导入客户姓名手机号）发出订单（Rabbitmq Redisson）共四个状态：派单 -》待接单 -》配送中 -》配送完成 如果订单被取消，管理端提示派送订单已被取消并恢复状态到派单。 
## 项目介绍
