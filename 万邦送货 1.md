...
7. **管理端点击派单，RabbitMQ 发送 3 分钟有效的“待接单”消息**
8. **司机端收到 RabbitMQ 消息**，可以在 3 分钟内抢单（Redisson 分布式锁防止并发）
9. **如果 3 分钟内无人接单**
    - RabbitMQ 进入 **死信队列**
    - 订单状态仍为 **待接单**
    - **司机端“待接单”列表可以看到订单，司机可随时接单**
10. **司机端随时接单，仍然使用 Redisson 分布式锁**
11. **如果订单超时太久（如 10 分钟以上），需要自动派单**
    - 选择 **XXL-Job 定时检查数据库**，自动指派司机
